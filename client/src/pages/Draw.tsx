...
export default function DrawingPad() {
    ...
    const _saved = (() => { try { return JSON.parse(localStorage.getItem("drawpad_v1") ?? "{}"); } catch { return {}; } })();
    ...
    const saveToLocal = useCallback(() => {
        try {
            localStorage.setItem("drawpad_v1", JSON.stringify({ shapes, color, strokeWidth, fillColor, opacity, rounded, roundedRadius, bgColor }));
            setSavedToast(true);
            setTimeout(() => setSavedToast(false), 2000);
        } catch { /* quota exceeded, ignore */ }
    }, [shapes, color, strokeWidth, fillColor, opacity, rounded, roundedRadius, bgColor]);

    // ── Auto-save to localStorage on every change 
    useEffect(() => {
        try { localStorage.setItem("drawpad_v1", JSON.stringify({ shapes, color, strokeWidth, fillColor, opacity, rounded, roundedRadius, bgColor })); }
        catch { /* quota exceeded, ignore */ }
    }, [shapes, color, strokeWidth, fillColor, opacity, rounded, roundedRadius, bgColor]);
            ...
            <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
                ...
                <div style={{height:44,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 12px",background:"#0d1117",borderBottom:"1px solid #1e293b",flexShrink:0,gap:8}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
                        ...
                        <button onClick={saveToLocal} style={{display:"flex",alignItems:"center",gap:5,background:savedToast?"#166534":"#0f172a",border:`1px solid ${savedToast?"#16a34a":"#1e293b"}`,color:savedToast?"#4ade80":"#64748b",padding:"5px 11px",borderRadius:6,fontSize:11,cursor:"pointer",fontFamily:"inherit",fontWeight:500,transition:"all 0.2s"}}>
