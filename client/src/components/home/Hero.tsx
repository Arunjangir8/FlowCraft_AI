...
import { EASE, Magnetic, Reveal, SplitLines, useIsMobile } from "./motion";
...
/** Mobile: plain stacked layout, mock reveals in on scroll — no pin,
 *  no scroll-jacked transforms. Sidesteps mobile Safari's dynamic
 *  toolbar breaking `position: sticky` + scroll-progress math. */
function HeroMobile() {
  return (
...
/** Desktop: pinned hero scene — the copy holds still while the
 *  canvas-window mock rises from the bottom edge and drifts up over
 *  it as you scroll, the product literally covering the pitch. The
 *  scene spans 200vh; the inner viewport is sticky, so the takeover
